...
We then show that LDpred2 compares favorably to other polygenic score methods.
...
For example, in simulations with 300 causal variants, a mean AUC of 73.5\% is obtained with LDpred1 while a value of 81.9\% is obtained with LDpred2 [TODO: VERIF NEW NUMBERS].
In these scenarios, all 3 non-infinitesimal models of LDpred2 perform equally well.
As expected, LDpred1 performs poorly in HLA scenarios, i.e.\ when causal variants are sampled in a region with variants in long-range LD. 
...
For PRCA and TD1, LDpred2-auto perform much worse than LDpred2, otherwise it performs similarly [TODO: CHANGE].
...
When comparing to other methods, we only report the best LDpred2 model (chosen using the validation set). 
...
As an example, it takes around two hours in total to run LDpred2-inf, LDpred2-grid (102 hyper-parameter values parallelized over 8 cores) and LDpred2-auto (30 initial values for $p$ parallelized over 8 cores) for a chromosome with 100,000 variants.
...
When comparing to recently derived methods that showed higher predictive performance as compared to the previous version of LDpred, LDpred2 performs favorably, providing the highest predictive performance for half of the real traits tested, and very good predictive performance for the other half.
...
Further investigation is needed to determine whether doing so would be beneficial for other methods such as LDpred2, lassosum and PRS-CS, which may break some of the assumptions behind their respective models.
Selection of variants is therefore a direction of future research for us.

[TALK ABOUT QUALITY CONTROL ON AF? -> ON SD, NEED A SUPP NOTE] 
[ALSO TALK ABOUT MODEL FOR BINARY DATA]
...
We defined phenotypes as in \cite{prive2019making}. For details, please refer to our R code (Software and code availability section). [TODO: TALK ABOUT QC HERE]
...
Using the regression would also enable adjusting for covariates in this step.
