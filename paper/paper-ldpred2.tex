...
For T1D, T2D, BRCA and PRCA, all main LDpred2 models perform much better than LDpred1. These improvements are also significant for MDD and CAD, albeit to a lesser extend. 
...
When comparing to other PGS methods, we only report the best LDpred2 model (chosen using the validation set). 
...
When comparing to recently derived methods that showed higher predictive performance compared to the previous version of LDpred, LDpred2 performs favorably, providing the highest predictive performance for half of the real traits tested, and very good predictive performance for the other half.
