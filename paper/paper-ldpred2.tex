...
We show that LDpred2 provides higher predictive performance than LDpred1, especially when there are causal variants in long-range LD regions or when the proportion of causal variants is small.
...
As a recall, LDpred1 has two options: LDpred1-grid where only $p$ is optimized, and LDpred1-inf.

We compare the two versions of LDpred using 6 simulation scenarios to understand in which cases it is important to use the new version. We also compare these two versions of LDpred using 8 binary phenotypes of major interest and for which there are published external summary statistics available and a substantial number of cases in the UK Biobank data.
...
In the first 4 simulation scenarios, a heritability of 40\% and a prevalence of 15\% are used. We simulate 300, 3000, 30,000 or 300,000 causal variants anywhere on the genome. 
...
In constrast, LDpred2 performs very well. We hypothesize that LDpred1 does not use a window size that is large enough to account for long-range LD such as in the HLA region.
...
We strongly discourage against removing these regions as suggested by others. Indeed, these regions, especially the HLA region, contain lots of variants associated with many traits, and are therefore very useful for prediction.
...
Overall, we test a grid of 102 different values in LDpred2 instead of 7 in LDpred1, we use 5 times as many iterations in the Gibbs sampler, we use a larger window size for computing correlation, yet LDpred2 is still faster than LDpred1.
LDpred2 is faster than LDpred1 because we provide an efficient parallel implementation in C++. The parallelization is performed on the grid of hyper-parameters, but can also be performed chromosome-wise, as each chromosome is processed independently in LDpred2 (see Methods).
...
When the sparse option is enabled, it provides models that are truly sparse, as compared to LDpred1 which outputs very small non-zero effect sizes \cite[]{cecile2019polygenic}.
