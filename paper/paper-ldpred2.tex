...
We benchmark predictive performance of LDpred2 against the previous version on simulated and real data, demonstrating substantial improvements in robustness, and now providing much more accurate polygenic scores compared to LDpred1.
We then show that LDpred2 also outperforms other polygenic score methods recently developed, such as lassosum, PRS-CS and SBayesR.
...
We show that LDpred2 provides higher predictive performance than LDpred1 (LDpred v1.0.0), especially when there are causal variants in long-range LD regions, when the proportion of causal variants is small, and when sample size is large.
...
Moreover, LDpred2-auto, which does not require any validation set, performs almost as well as LDpred2-grid (the main LDpred2 model which tunes hyper-parameters from a grid in a validation set), provided some quality control is performed on the summary statistics.
...
For this paper, to show the effect of this sparse option on predictive performance, we deliberately separate the LDpred2-grid method in two ``different'' methods that we call LDpred2-grid-nosp (sparse option always disabled) and LDpred2-grid-sp (sparse option always enabled) in the results.
...
[TODO: MAKE SURE ABOUT NAMING OF BOOTSTRAP TECHNIQUE + ADD CORRESPONDING TABLES IN SUPPMAT WHEN CODE OUT]
...
Yet, LDpred1-grid starts providing lower predictive performance than LDpred2-grid when sample size becomes larger. 
...
As in simulations, the sparse version of LDpred2-grid performs as well as the non-sparse version. Sparsity of resulting effects ranges from 19.3\% for RA to 54.4\% for Asthma [TODO: ADD REF TO TABLE THAT SHOWS THE SPARSITY OF THE CORRESPONDING MODEL WHEN OUT -> SEE SCREENSHOT IN SUPPMAT FOR NOW]
...
Similarly, PRS-CS-auto also underperforms for MDD (53.9\% for PRS-CS-auto vs AUC of 57.5\% for PRS-CS).
...
Indeed, except for LDpred2-inf applied to T1D, all other cases show similar or better performance for running all variants of LDpred2 genome-wide. 
...
[TODO: TALK ABOUT THE WRONG H2 ESTIMATION FOR CAD IN DISCU? -> CF. TABLE + SOME FIGURES IN SUPPMAT]
...
In LDpred2, we expand the grid of hyper-parameters examined with now more values for $p$ (21 instead of 7 by default in LDpred1) and for $h^2$ (3 instead of 1).
...
The second extension is LDpred2-auto, which automatically estimates values for hyper-parameters $p$ and $h^2$, therefore which does not require a validation set to choose hyper-parameters, making it an attractive option in many applications. 
LDpred2-auto almost always performs equally well as LDpred2-grid in simulations as well as in real data applications, provided that some quality control is performed on summary statistics (see Methods section ``Quality control of summary statistics'').
...
We also provide a sparse option for LDpred2-auto by running LDpred2-grid with only one model with the sparsity enabled and using the estimates of $p$ and $h^2$ from LDpred2-auto.
