...
Bars present the mean and 95\% CI of 10,000 non-parametric bootstrap replicates of the mean AUC from 10 simulations for each scenario. 
...
We can see that LDpred1-grid performs equally well as LDpred2-grid run genome-wide for the smallest GWAS sample sizes. 
Yet, LDpred1-grid starts to fail when sample sizes is larger. Indeed for the scenario with sample size of 120,000, LDpred1 provides a mean AUC of [TODO] while LDpred2 provides a mean AUC of [TODO].
Moreover, LDpred1 even performs slightly worse in the scenarios with a sample size of 300,000, while the performance of LDpred2 continues to increase steadily towards the maximum achievable of AUC of 82.5\% for a heritability of 40\% and a prevalence of 15\% \cite[]{wray2010genetic}.
...
These improvements are also significant for MDD and CAD, albeit to a lesser extent. 
...
LDpred2 performs best for Asthma, RA, T1D and TD2, while lassosum performs best for BRCA, CAD, MDD and PRCA.
...
[NEED TO INCLUDE FIGURE \ref{fig:AUC-auto} ABOUT AUTO MODELS]
...
Figure \ref{fig:AUC-simu2-ldpred2} shows that it can be beneficial to run all LDpred2 models genome-wide rather than per chromosome, especially when GWAS shample size is small.
For example, in the scenario with the sample size of 20,000, switching running LDpred2 per chromosome to running it genome-wide improves AUC of LDpred2-inf from [TODO] to, LDpred2-grid from to, and LDpred2-auto from to [TODO].
...
Indeed, except for LDpred2-inf applied to Type 1 Diabetes (T1D) for which running it per chromosome provides an AUC of [TODO] versus of [TODO] when run genome-wide, all other cases show similar or better performance for running all variants of LDpred2 genome-wide.
...
We simulate binary phenotypes with a heritability of $h^2=0.4$ (or $h^2=0.3$ when causal effects are all located in the HLA region of chromosome 6)  using a Liability Threshold Model (LTM) with a prevalence of 15\% \cite[]{falconer1965inheritance}. 
