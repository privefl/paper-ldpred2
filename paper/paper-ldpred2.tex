...
\abstract{	
In recent years the use of polygenic scores has become widespread.
LDpred is a popular and powerful method for deriving polygenic scores based on summary statistics and a Linkage Disequilibrium (LD) matrix only.
However, LDpred has several limitations that may result in limited predictive performance.
Here, we present LDpred2, a new version of LDpred that addresses these issues therefore providing higher predictive performance, while markedly improving its computational efficiency.
We provide this faster and more robust implementation of LDpred in R package bigsnpr, as well as two new options in LDpred2: ``sparse'' and ``auto''.
}
...
As an example, it takes around two hours in total to run LDpred2-inf, LDpred2-grid (108 hyper-parameter values parallelized over 8 cores) and LDpred2-auto for a chromosome with 100K variants.
...
Yet, we do not know yet why LDpred2-auto performs poorly specifically for PRCA. More investigations need to be performed to understand these poor results, and to find if this issue can be fixed. Solutions that come to mind include for example running multiple chains with different initial values of $p$ and keeping the one with the best convergence properties.
...
We use the UK Biobank data for both real data analyses and simulations \cite[]{bycroft2018uk}. We restrict individuals to White-British individuals used in the PC computation of the UK Biobank (i.e.\ unrelated and quality-controlled). We restrict variants to HapMap3 variants.
...
Thus, we can derive $\text{sd}(g) \approx \dfrac{\text{sd}(y)}{\text{se}(\hat{\gamma}) ~ \sqrt{N}}$ and $\left(\text{sd}(g) ~ \hat{\gamma}\right) \approx \dfrac{\hat{\gamma}}{\text{se}(\hat{\gamma})} \dfrac{\text{sd}(y)}{\sqrt{N}}$.
...
As $\text{sd}(y)$ is the same for all variants, it is cancelled out by $S^{-1}$ and $S$. Then it justifies the use of Z-scores ($\hat{\gamma} ~/~ \text{se}(\hat{\gamma})$) divided by $\sqrt{N}$ as input for LDpred. Then, the effect sizes that LDpred outputs need to be scaled by multiplying by $\left(\text{se}(\hat{\gamma}) ~ \sqrt{N}\right)$.
...
\begin{algorithm}[H]
\caption{LDpred, with hyper-parameters $p$ and $h^2$, and summary statistics $\boldsymbol{\hat{\gamma}}$, $\boldsymbol{\text{se}(\hat{\gamma})}$ and $\boldsymbol{N}$}\label{algo:ldpred}
\begin{algorithmic}[1]
	\State $\boldsymbol{\hat{\beta}} \gets \dfrac{\boldsymbol{\hat{\gamma}}}{\boldsymbol{\text{se}(\hat{\gamma})} \cdot \sqrt{\boldsymbol{N}}}$ \Comment{See previous section}
	...
	\State Return $\boldsymbol{\Omega} \cdot  \boldsymbol{\text{se}(\hat{\gamma})} \cdot \sqrt{\boldsymbol{N}}$ \Comment{See previous section}
\end{algorithmic} 
\end{algorithm}
%\end{minipage}
...
Due to complexity reasons, we could not derive a Bayesian estimator of $h^2$. Instead, we estimate $h^2 = \boldsymbol{\beta}^T \boldsymbol{K} \boldsymbol{\beta}$, where $\boldsymbol{K}$ is the correlation matrix.
...
Moreover, since the inverse of a block-diagonal matrix is formed from the inverse of each block, it should be safe to run LDpred2 for each chromosome and then combine the results. Combining results from all chromosomes requires using some appropriate scaling (see tutorial).
