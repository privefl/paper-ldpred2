...
\begin{table}[ht]
	...
	\begin{tabular}{|l|c|c|c|c|c|}
		...
		\hline
	\end{tabular}
\caption{Values from figures 2 and S2.}
\end{table}
...
\begin{table}[ht]
	...
	\begin{tabular}{|l|c|c|c|c|c|c|c|c|}
		...
		\hline
	\end{tabular}
\caption{Values from figures 3 and S3.}
\end{table}
...
\begin{tabular}{|l|c|c|c|c|c|c|c|c|}
	...
	\hline
\end{tabular}
\caption{Values from figures 4 and S1.}
\end{table}
...
\begin{figure}[htb]
	...
	\caption{Both ``grid'' and ``auto'' variants are compared in real data applications for LDpred2, lassosum, PRS-CS and SBayesR. Note that SBayesR is such an ``auto'' model, but does not have any ``grid'' counterpart.
		...
		See corresponding values in table S4.
	}
	\label{fig:AUC-auto}
\end{figure}
...
\begin{figure}[htb]
	...
	Bars present AUC values on the test set of UKBB (mean and 95\% CI from 10,000 bootstrap samples).
	See corresponding values in table S3.
	}
...
\begin{table}[ht]
	...
	\begin{tabular}{|l|c|c|c|c|c|c|}
		...
		\hline
		Asthma & 0.104 & 0.544 & 0.000738 & 0.0534 & 0.15 & 0.064 \\ 
		BRCA & 0.127 & 0.414 & 0.00474 & 0.141 & 0.0758 & 0.136 \\ 
		CAD & 0.0819 & 0.441 & 0.000978 & 0.0428 & 0.0619 & 0.0387 \\ 
		MDD & 0.0923 & 0.377 & 0.0675 & 0.0877 & 0.0893 & 0.089 \\ 
		PRCA & 0.188 & 0.516 & 0.00181 & 0.189 & 0.0546 & 0.164 \\ 
		RA & 0.416 & 0.193 & 0.0774 & 0.562 & 0.0286 & 0.406 \\ 
		T1D & 0.955 & 0.498 & 0.00482 & 1.4 & 0.00244 & 0.575 \\ 
		T2D & 0.148 & 0.498 & 0.0031 & 0.131 & 0.0512 & 0.112 \\ 
		\hline
	\end{tabular}
\caption{Parameters obtained when running LDpred2 genome-wide for all 8 real traits analyzed here. h2\_ldsc\_obs: SNP heritability estimate (on the observed scale) from LD score regression (with intercept); sparsity\_grid: sparsity of resulting effects from LDpred2-grid-sp; p\_auto: estimate from LDpred2-auto of the proportion of causal variants; h2\_auto\_obs: SNP heritability estimate from LDpred2-auto; prev\_pop: assumed population prevalence based on prevalence in the UK Biobank; h2\_auto\_liab: transformation of h2\_auto\_obs on the liability scale when assuming population prevalence prev\_pop. \label{tab:ldpred2-params}}
\end{table}

\begin{figure}[htb]
	\centerline{\includegraphics[width=0.8\textwidth]{CAD-grid}}
	\caption{Results for all 126 parameter combinations of LDpred2-grid run genome-wide when applied to summary statistics for CAD.}
	\label{fig:CAD-grid}
\end{figure}
...
\begin{subfigure}[b]{\textwidth}
	\centerline{\includegraphics[width=0.75\textwidth]{CAD-auto}}
\end{subfigure}

\begin{subfigure}[b]{\textwidth}
	\centerline{\includegraphics[width=0.75\textwidth]{CAD-auto2}}
\end{subfigure}
\caption{Paths of LDpred2-auto-gwide parameters, the SNP heritability $h^2$ and the proportion of causal variants $p$, for CAD. We show these for two different starting values for $p$, with an initial $p$ of 0.0001 at the top and initial $p$ of 0.1 at the bottom.}
\label{fig:CAD-auto}
\end{figure}
...
\begin{figure}[htbp]
	...
	\caption{Standard deviations derived from summary statistics of breast cancer (BRCA) based on equation (3) versus the standard deviations of genotypes of individuals in the validation set. Coloring shows the quality control applied in this paper.}
	\label{fig:sd-approx-brca}
\end{figure}

\begin{figure}[htbp]
	\centerline{\includegraphics[width=0.9\textwidth]{sd-approx-CAD}}
	\caption{Standard deviations derived from summary statistics of coronary artery disease (CAD) based on equation (3) versus the standard deviations of genotypes of individuals in the validation set. Coloring shows the quality control applied in this paper.}
	\label{fig:sd-approx-cad}
\end{figure}

\begin{figure}[htbp]
	\centerline{\includegraphics[width=0.9\textwidth]{sd-approx-asthma}}
	\caption{Standard deviations derived from summary statistics of asthma based on equation (3) versus the standard deviations of genotypes of individuals in the validation set. Coloring shows the quality control applied in this paper.}
	\label{fig:sd-approx-asthma}
\end{figure}
...
\subsection*{Providing an LD reference}
